...
"""
    This program was edited to be functional
    and to be easier to use for our requirements
"""
########################################################################
import getopt
...
def main(argv):
    ...
    try:
        ...
        print(
            '\033[1m' + 'python G4Hunter.py -i <inputfile> -o <outputrepository> -w <window> -s <score threshold>\n' + '\033[0;0m')
        sys.exit(1)
    ...
    for opt, arg in opts:
        if opt in ('-h', "--help"):
            ...
            print('\n G4Hunter takes into account G-richness and G-skewness of a given sequence and gives a '
                  'quadruplex propensity score as output.')
            print('To run G4Hunter use the command line: \n')
            print(
                '\033[1m' + 'python G4Hunter.py -i <inputfile> -o <outputrepository> -w <window> -s <score threshold>\n' + '\033[0;0m')
            sys.exit()
        elif opt in ("-i", "--ifile"):
...
class Soft(object):
    ...
    def ReadFile(self, Filein):
        ...
        f = open(Filein, 'r')
        for line in f:
            LHeader.append(str(i))
            i += 1
            ListSeq.append(line[:-1])
    ...
    def BaseScore(self, line):
        ...
        while item < len(line):
            ...
            #   liste.append(0)
            #   item=item+1
            elif (item < len(line) and line[item] != "G" and line[item] != "g" and line[item] != "C" and line[
                item] != "c"):
                liste.append(0)
                item = item + 1
    ...
    def GetG4(self, line, fileout, liste, threshold, k, header, file):
        LG4 = []
        SEQ = ">" + header + "\n Start \t End \t Sequence\t Length \t Score\n"
        fileout.write(SEQ)
        for i in range(len(liste)):
            if liste[i] >= float(threshold) or liste[i] <= - float(threshold):
                seq = line[i:i + k]
                LG4.append(i)
                self.Write(fileout, i, k, 0, 0, seq, k, liste[i])
                fileout.write("\n")
        file.write(str(max(liste + [0])) + '\n')
        return LG4

    def WriteSeq(self, line, fileout, liste, LISTE, header, F, file):
        i, k, I = 0, 0, 0
        a = b = LISTE[i]
        MSCORE = []
        ...
        if len(LISTE) > 1:
            ...
            while i < len(LISTE) - 2:
                if c == b + 1:
                    ...
                    scores.append(round(np.mean(liste2), 2))
            ...
            scores.append(liste[a])
            MSCORE.append(abs(liste[a]))
            fileout.write("\t")
            fileout.write(str(I))
            fileout.write("\n")
        # file.write(str(max(scores + [0])) + '\n')
        return MSCORE
...
if __name__ == "__main__":
    ...
    soft1 = Soft()
    ScoreListe, DNASeq, NumListe, HeaderListe = soft1.GFinder(inputfile, window)
    # added another output file for easier evaluation
    file_out = open('G4Hunter_output.txt', 'w')
    for i in range(len(DNASeq)):
        G4Seq = soft1.GetG4(DNASeq[i], Res1file, ScoreListe[i], float(score), int(window), HeaderListe[i],
                            file_out)
        if len(G4Seq) > 0:
            MSCORE = soft1.WriteSeq(DNASeq[i], Res2file, ScoreListe[i], G4Seq, HeaderListe[i], int(window),
                                    file_out)
        else:
            file_out.write('0\n')
    # plot.append(MSCORE)
